...
static void test_cgrad_tensor_layout_init_and_copy(void **state) {
    ...
    assert_int_equal(cgrad_tensor_layout_init(&l1, shape, 3), 0);
...
static void test_cgrad_tensor_layout_flat_index(void **state) {
    ...
    assert_int_equal(cgrad_tensor_layout_init(&l, shape, 2), 0);

    // Valid index (for last 2 dims)
    uint32_t indices_valid[2] = {3, 2};
    size_t idx = 0;
    int err = cgrad_tensor_layout_flat_index(&l, indices_valid, 2, &idx);
    ...
    err = cgrad_tensor_layout_flat_index(&l, indices_oob, 2, &idx);
...
static void test_cgrad_tensor_layout_transpose(void **state) {
    ...
    cgrad_tensor_layout_init(&l, shape, TENSOR_DIM);
    ...
    cgrad_tensor_layout_transpose(&l, perm, TENSOR_DIM);
...
static void test_cgrad_tensor_layout_transpose_duplicate_dim(void **state) {
    ...
    int err = cgrad_tensor_layout_transpose(&l, perm, TENSOR_DIM);
...
static void test_cgrad_tensor_layout_reshape(void **state) {
    (void)state;
    cgrad_tensor_layout l;
    ...
    assert_int_equal(cgrad_tensor_layout_reshape(&l, new_shape1, 3), CGRAD_SUCCESS);
    for (int i = 0; i < TENSOR_DIM - 3; ++i) assert_int_equal(l.shape[i], 1);
    ...
    assert_int_equal(cgrad_tensor_layout_init(&l, shape2, 3), 0);
    int32_t new_shape2[3] = {4, -1, 2}; // -1 should be 3
    assert_int_equal(cgrad_tensor_layout_reshape(&l, new_shape2, 3), CGRAD_SUCCESS);
    assert_int_equal(l.shape[TENSOR_DIM - 2], 3);

    // Reshape with -1 (infer) and reduce ndim
    uint32_t shape3[3] = {2, 3, 4};
    assert_int_equal(cgrad_tensor_layout_init(&l, shape3, 3), 0);
    int32_t new_shape3[2] = {4, -1}; // -1 should be 6
    assert_int_equal(cgrad_tensor_layout_reshape(&l, new_shape3, 2), CGRAD_SUCCESS);
    assert_int_equal(l.shape[TENSOR_DIM - 1], 6);

    // Error: product mismatch
    uint32_t shape4[3] = {2, 3, 4};
    assert_int_equal(cgrad_tensor_layout_init(&l, shape4, 3), 0);
    int32_t bad_shape[3] = {2, 2, 2}; // 2*2*2=8 != 24
    assert_int_equal(cgrad_tensor_layout_reshape(&l, bad_shape, 3), CGRAD_LAYOUT_ERR_RESHAPE_INVALID_SHAPE);

    // Error: more than one -1
    int32_t bad_shape2[3] = {-1, -1, 2};
    assert_int_equal(cgrad_tensor_layout_reshape(&l, bad_shape2, 3), CGRAD_LAYOUT_ERR_RESHAPE_INVALID_SHAPE);

    // Error: non-regular layout
    uint32_t shape5[3] = {2, 3, 4};
    assert_int_equal(cgrad_tensor_layout_init(&l, shape5, 3), 0);
    // Make strides irregular
    if (TENSOR_DIM > 2) l.strides[TENSOR_DIM - 2] = 7;
    int32_t new_shape5[3] = {4, 3, 2};
    assert_int_equal(cgrad_tensor_layout_reshape(&l, new_shape5, 3), CGRAD_LAYOUT_ERR_NOT_REGULAR);

    // Stride scaling: regular but step != 1
    uint32_t shape6[3] = {2, 3, 4};
    assert_int_equal(cgrad_tensor_layout_init(&l, shape6, 3), 0);
    // Make strides a constant multiple of contiguous (e.g., *2)
    for (int i = 0; i < TENSOR_DIM; i++) l.strides[i] *= 2;
    int32_t new_shape6[3] = {4, 3, 2};
    assert_int_equal(cgrad_tensor_layout_reshape(&l, new_shape6, 3), CGRAD_SUCCESS);
    ...
    assert_int_equal(cgrad_tensor_layout_init(&l, shape7, TENSOR_DIM), 0);
    int32_t collapse_all[1] = {-1};
    assert_int_equal(cgrad_tensor_layout_reshape(&l, collapse_all, 1), CGRAD_SUCCESS);
...
int run_cgrad_layout_tests(void) {
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test(test_cgrad_tensor_layout_reshape),
    };
